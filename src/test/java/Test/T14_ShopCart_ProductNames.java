...
public class T14_ShopCart_ProductNames extends BaseTest {
    ...
    public void shoppingCart() {
        ...
        //Assert that the names of the products added are present in the cart
        List<String> expectedProducts = List.of("Alfie si gradina de legume", "Alfie si prietenul lui cel mai bun");
        List<WebElement> actualProducts = helpMethods.getProductElements();
        for (int i = 0; i < actualProducts.size(); i++) {
            String actualProductName = actualProducts.get(i).getAttribute("title");
            Assert.assertTrue(expectedProducts.contains(actualProductName), "Item not found in cart: " + actualProductName);
        }
